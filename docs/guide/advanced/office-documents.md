...
The easiest way to create a document is to describe what you want:
...
:::tip Start Simple
You do not need to specify exact slide layouts or cell formatting. The agent makes reasonable choices. You can always refine by saying "make the title slide more prominent" or "add a chart for the monthly data."
:::

## Template Workflow for Presentations

For professional or corporate presentations, Obsilo supports a template-based pipeline. This is the most powerful way to create presentations that match your organization's design.

### How It Works

1. **Provide a template** -- Drop a `.pptx` template file into your vault
2. **Agent analyzes it** -- The agent scans every slide layout, placeholder, and shape in the template (stored as a TemplateCatalog)
3. **Agent plans content** -- An internal LLM call maps your source material to the template's structure, planning content for each slide and shape
4. **Agent generates** -- The final presentation is built using your template's exact design

### Step by Step

1. Attach or mention your template: *"Use @corporate-template.pptx to create a presentation about our product roadmap"*
2. The agent runs `plan_presentation` internally -- you will see it in the activity block
...
Obsilo can parse existing office files and use their content in conversations. This works for:
...
The agent uses the `read_document` tool to parse the file, then works with the extracted content just like any note.
...
Visual QA only works if LibreOffice is installed and accessible from the command line. Without it, the agent will skip the visual check and rely on structural validation only.
:::
...
3. **Use templates for consistency.** If you create presentations regularly, invest in a good template -- the agent will reuse it perfectly every time.
...
- [Multi-Agent & Tasks](/guide/advanced/multi-agent) -- Delegate complex document tasks to sub-agents
