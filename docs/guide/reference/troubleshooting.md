...
Solutions for the most common Obsilo issues. If your problem is not listed here, check the **Debug** tab in settings or ask in the community forum.
...
| Auto-index disabled | If auto-index is off, new or changed notes are not indexed. Enable it or rebuild manually. |
...
| Tool permission denied repeatedly | The agent asks for approval but you have not responded. Approve or deny to let it continue. |
...
| Large context window | Enable context condensing to prevent the conversation from growing too large. |
...
| Wrong memory model | If the memory model is not configured or offline, extraction silently fails. Check **Settings > Memory > Memory model**. |
