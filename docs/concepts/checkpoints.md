...
Checkpoints are stored as commits in a **shadow git repository** that is separate from any git repository you might have running on your vault. The shadow repo lives outside the vault at `{vault-parent}/vault-operator-shared/checkpoints/`, so Obsidian Sync and iCloud do not replicate it, and it never touches your own `.git/` folder.
...
The shadow repo is created automatically on the first task that writes. There is nothing to configure. Older installs upgraded from `.obsidian-agent` (the pre-v2.13 folder name) keep working without manual migration.
...
- **The shadow repo itself.** The plugin manages it, you do not need to touch it. If you ever need to reset it, delete `{vault-parent}/vault-operator-shared/checkpoints/` and the next task will create a fresh one.
...
The plugin does not prune checkpoints automatically. If disk usage becomes a concern, delete `{vault-parent}/vault-operator-shared/checkpoints/`. The next task will start a fresh shadow repo.
