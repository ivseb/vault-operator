...
description: How Vault Operator unifies history and memory across Claude.ai, Claude Code, ChatGPT, Perplexity, and Vault Operator itself.
---

# Unified Chat Memory (UCM)

You probably do not stay inside one chat tool. A coding question goes to Claude Code, a research question to Perplexity, a long brainstorm to Claude.ai or ChatGPT, and the writing happens in Vault Operator. Each tool has its own memory and its own history. Insights from one do not transfer to another.

Unified Chat Memory (UCM) is the layer in Vault Operator that fixes this. It is not a separate product or a future plan. It is the cross-surface side of Memory v2 that ships today. UCM turns Vault Operator into the single place where conversations from your other AI tools land, where their facts get stored, and where you search across all of them at once.

## What it does

UCM lets the other AI tools you use:

- **Save conversations into Vault Operator's history.** A finished chat in Claude Code or ChatGPT lands in Vault Operator's history sidebar, tagged with the source it came from.
- **Save facts into Vault Operator's memory.** A single insight ("I prefer pnpm over npm", "the production database is on Neon") becomes a fact in the same store that powers Vault Operator's own memory.
- **Recall facts and search history from inside the other tool.** The other tool can ask Vault Operator "what do you remember about X?" and get back facts that were learned in any of the connected tools.
...
The connecting glue is the [Model Context Protocol](./mcp-architecture). Vault Operator runs an MCP server. Any AI tool that speaks MCP (Claude Desktop, Claude Code, custom GPTs in Developer Mode, Perplexity Spaces with MCP support) can connect to it.
...
| Tool | What it does |
|------|--------------|
| `save_conversation` | Copies a chat from another tool into Vault Operator's history sidebar |
...
UCM treats this as a living document. When the same source tool calls `save_conversation` again within 30 minutes from the same MCP session, the new turns are appended to the existing conversation instead of creating a fresh one. The other tool does not need to track the conversation ID. Vault Operator detects the active conversation and either computes the delta from a full transcript or appends the new turns as-is.
...
**Auto** runs the same Memory v2 extraction pipeline that Vault Operator uses for its own conversations. Facts land in the store as soon as the conversation is closed.
...
The history sidebar groups conversations into source tabs. Each tab only appears when at least one conversation of that source exists, so a fresh install with only Vault Operator chats shows a clean single-tab view.

| Tab | Contents |
|-----|----------|
| All | Everything across sources |
| Vault Operator | Plugin sidebar conversations |
...
Threads also exist purely inside Vault Operator. Every conversation in the sidebar can be linked into a thread without touching MCP at all.
...
UCM is the **shared store and protocol** that makes cross-surface memory possible. It is not a hosted service, a separate app, or a future MCP package you install elsewhere. The Vault Operator plugin is the server. Other tools are clients. If you uninstall Vault Operator, UCM goes with it.

The shared facts and conversations live in your local `memory.db` and `history.db`. Nothing goes to a cloud, no telemetry, no third-party storage. UCM extends Vault Operator's local-first design across tools without breaking it.
