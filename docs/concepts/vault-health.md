...
- **Enable external sources** is the master switch for any web traffic from the freshness system. Both Stufe 3 paths refuse to run while it is off.
- **Weekly automatic check** turns the Stufe 3 job on or off (default off). The run covers the whole vault, driven by note age; **Never-check clusters** and **Exclude paths** opt clusters and folders out, and the **Freshness scan scope** block previews what the next run would cover.
- **Run freshness check now** starts a one-off Stufe 3 run with the same pipeline and budget.
