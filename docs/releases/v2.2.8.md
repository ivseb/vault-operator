...
- **MCP clients must authenticate:** existing MCP integrations (Claude Desktop) will auto-reconnect via the stdio proxy worker, which reads the token from `~/.obsidian-agent/mcp-token`. Manual HTTP clients need to include an `Authorization: Bearer <token>` header.
