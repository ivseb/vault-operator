...
- **Skill definition:** the skills folder `{skills_root}/{name}/`. The SKILL.md and its `scripts/` `references/` `assets/`. Only this path is discovered.
- **Skill state and sidecar data:** `{skill_data_root}/{name}/`. Dashboards, caches, generated data files the skill reads back. Keeping this outside `skills/` avoids snapshot churn on the skill folder.
...
The agent folder is hidden (its name is a user setting, default `.vault-operator`, always dot-prefixed), so Obsidian Sync excludes it. Anything the user is meant to see and sync belongs in the visible vault, not under the agent folder. A daily-briefing skill, for example, hides its data under `skill-data/` but writes the `Daily Briefing.md` note into the visible vault.
...
For skill state under the agent folder, write through `ctx.vault.write` (sandbox) or `write_file` (body), which are hidden-aware and atomic. For user output, use a normal visible folder. Do not point `append_to_file` at a dot-path.
