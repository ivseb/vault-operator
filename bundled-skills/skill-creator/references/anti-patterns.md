...
**Wrong folder path.** A skill written anywhere other than the skills folder `{skills_root}/{name}/` is invisible to discovery. `init_skill` is the only valid seed. A half-finished skill at a wrong path is garbage from a broken run; delete it after the real one lands.
