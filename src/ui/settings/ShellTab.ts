...
export class ShellTab {
    ...
    private buildBuiltinRecipesSection(containerEl: HTMLElement): void {
        ...
        for (const recipe of BUILT_IN_RECIPES) {
            ...
            new Setting(containerEl)
                .setName(recipe.name)
                .setDesc(t('settings.shell.recipeDesc', { description: recipe.description, binary: recipe.binary }))
                .addToggle((t) =>
    ...
    private buildAdaptivePluginApiSection(containerEl: HTMLElement): void {
        addSectionHeading(
            containerEl,
            t('settings.shell.headingAdaptive'),
            { body: t('settings.shell.sectionAdaptiveInfo') },
        );
        ...
        new Setting(containerEl)
            .setName(t('settings.shell.defaultTimeout'))
            .setDesc(t('settings.shell.defaultTimeoutDesc'))
            .addText((tx) => tx
        ...
        if (overrideKeys.length > 0) {
            containerEl.createEl('p', {
                ...
                text: t('settings.shell.perPluginOverrides'),
            });
            for (const pluginId of overrideKeys) {
                new Setting(containerEl)
                    .setName(pluginId)
                    .setDesc(t('settings.shell.customTimeoutDesc', { value: perPlugin[pluginId] }))
                    .addText((tx) => tx
                    ...
                    .addButton((b) => b
                        .setButtonText(t('settings.shell.remove'))
                        .onClick(async () => {
        ...
        new Setting(containerEl)
            .setName(t('settings.shell.addOverride'))
            .setDesc(t('settings.shell.addOverrideDesc'))
            .addText((tx) => tx
                .setPlaceholder(t('settings.shell.addOverrideIdPlaceholder'))
                .onChange((v) => { newPluginId = v.trim(); }))
            .addText((tx) => tx
                .setPlaceholder(t('settings.shell.addOverrideTimeoutPlaceholder'))
                .onChange((v) => { newTimeout = v; }))
            .addButton((b) => b
                .setButtonText(t('settings.shell.addButton'))
                .onClick(async () => {
        ...
        new Setting(containerEl)
            .setName(t('settings.shell.autoPromoteThreshold'))
            .setDesc(t('settings.shell.autoPromoteThresholdDesc'))
            .addText((tx) => tx
        ...
        if (sorted.length > 0) {
            containerEl.createEl('p', {
                cls: 'agent-settings-desc',
                text: t('settings.shell.approvalCountsTitle'),
            });
            for (const [key, count] of sorted) {
                const safe = (this.plugin.settings.pluginApi?.safeMethodOverrides ?? {})[key];
                new Setting(containerEl)
                    .setName(key)
                    .setDesc(t('settings.shell.approvedCount', { count }) + (safe ? t('settings.shell.autoPromotedSuffix') : ''));
            }
        }
    }
}
