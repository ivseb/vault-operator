...
describe('validateProviderUrl', () => {
    ...
    describe('bedrock gatewayMode (FEAT-26-07)', () => {
        it('accepts an enterprise APIM host like apim.example.com when gatewayMode is true', () => {
            expect(() =>
                validateProviderUrl(
                    'bedrock',
                    'https://gateway.apim.example.com/genai/bedrock',
                    { gatewayMode: true },
                ),
            ).not.toThrow();
        });
        ...
        it('regression: without gatewayMode, apim.example.com still fails the strict bedrock allow-list', () => {
            expect(() =>
                validateProviderUrl(
                    'bedrock',
                    'https://gateway.apim.example.com/genai/bedrock',
