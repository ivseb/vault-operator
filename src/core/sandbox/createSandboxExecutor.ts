 ...
 * The deprecated `'process'` value no longer routes to the vm backend, so no
 * platform can select the escapable path; ProcessSandboxExecutor is left in
 * the tree but is no longer imported here and therefore no longer bundled.
 *
