...
10. EXTERNALIZED RESULTS. Large tool results (search results, file contents) may be saved to temporary files for efficiency. When you see a tool result containing "Full results saved to:" or "Use read_file(...) to see full content", the complete data is available in that file. Use read_file to load it ONLY if you need the full details — the compact summary in the result is often sufficient.`;
}
