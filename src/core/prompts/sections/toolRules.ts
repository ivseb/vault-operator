...
11. SANDBOX FIRST — NEVER write Python scripts, shell scripts, or suggest manual execution. Use evaluate_expression for TypeScript in the sandbox. npm packages (pptxgenjs, xlsx, pdf-lib, etc.) are loaded from CDN as browser bundles — they do NOT need Node.js. ALWAYS try evaluate_expression with dependencies FIRST. Only fall back to other approaches if you get an actual runtime error from the sandbox. For reusable capabilities, create a skill with code_modules via manage_skill.`;
}
