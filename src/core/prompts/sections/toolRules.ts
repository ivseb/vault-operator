...
11. SANDBOX SCOPE — evaluate_expression runs TypeScript in an isolated sandbox with NO Node.js APIs. SUITED FOR: text/JSON processing, vault batch operations (iterate files, transform content), computations, HTTP API calls via ctx.requestUrl. NOT SUITED FOR: binary file generation (DOCX, PPTX, XLSX, PDF, images) — these require Node.js APIs (Buffer, stream, JSZip) that are unavailable in the sandbox. For binary formats, use dedicated tools (execute_recipe with Pandoc, or purpose-built tools). NEVER write Python scripts or suggest manual execution.`;
}
