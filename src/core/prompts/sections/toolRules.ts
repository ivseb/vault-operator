...
4. READ BEFORE EDITING (once). Use read_file before editing a file you have NOT already read this task. Do NOT re-read after your OWN edit_file / append_to_file / set_block_anchors: the untouched regions are byte-identical, so continue from your in-context copy plus the diff the tool returned. (update_frontmatter re-serialises YAML -- use the frontmatter block it returns as the new truth for that region.)
5. PREFER edit_file OVER write_file for changes to existing files.
5b. PREFER update_frontmatter OVER edit_file for YAML frontmatter VALUE changes on a single note. update_frontmatter is atomic. It preserves all existing fields, handles arrays correctly, and creates the frontmatter block if none exists. Combine all frontmatter updates into a single call. For STRUCTURAL or BULK frontmatter operations across many notes (rename/delete a property key, rename values, set a value across a filter selection), use call_plugin_api("frontmatter-operator", "<method>", ...) instead -- see the tool routing section.
