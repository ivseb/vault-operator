...
| `image-size` pulled in by `pptxgenjs` (GHSA-w3rx-r6r6-pgpr, GHSA-5p2g-fcmc-qvqq) | Warning | Build-time only, unreachable at runtime, and no upstream fix exists. `pptxgenjs` is a `devDependency`: it is listed in esbuild's `external` array, so it never enters `main.js`, and it is compiled only into `office-bundle.js`, an optional asset published on a separate release tag and downloaded on demand. The runtime dependency tree is therefore free of it, which `npm audit --omit=dev` confirms with zero findings. Both advisories cover infinite loops in the ICNS, JXL and HEIF parsers of `image-size` at range `<= 2.0.2`; 2.0.2 is the newest published version (`npm view image-size dist-tags` returns `latest: 2.0.2`), so there is nothing to upgrade to, and npm's only proposed remediation is a major downgrade to `pptxgenjs@1.1.5`. The parser never enters the shipped bundle, on four checks anyone can repeat: (1) none of the four dist files that `pptxgenjs@4.0.1` ships (`pptxgen.cjs.js`, `pptxgen.es.js`, `pptxgen.bundle.js`, `pptxgen.min.js`) contains the string `image-size`; (2) `pptxgenjs/package.json` maps `"image-size": false` in its `browser` field, and `office-bundle.js` is built with esbuild `platform: "browser"` (`esbuild.config.mjs`, `generateOfficeBundles`), so the field applies and the import resolves to an empty stub; (3) the library's only dimension helper, `getSizeFromImage`, sits inside a block comment marked "FIXME: TODO: currently unused", and its single call site is commented out as well; (4) the built `office-bundle.js` and `main.js` contain zero `image-size`, `icns` or `jxl` markers. Checks (1) and (2) are pinned by `src/core/assets/__tests__/pptxgenjsImageSizeUnreachable.test.ts`, so a `pptxgenjs` upgrade that makes the parser reachable fails the suite instead of shipping. If that test goes red, the answer is a magic-byte prefilter at the single point where image bytes reach `pptxgenjs` (`CreatePptxTool.addImage`), not a downgrade. |
